...
class Actor(nn.Module):
    def __init__(self, state_dim, action_dim, M, N, K, power_t, group, device, max_action=1):
        ...
        self.Group = group
        print("@@@@@@@@@@@@@@@@@")
        print('inside actor Power ')
        print(self.power_t) # Print the power_t value   
        print("@@@@@@@@@@@@@@@@@")
    ...
    def compute_phase(self, a):
        ...
        Phi_real = a[:, -2*self.N*self.N: -self.N*self.N].detach()
        Phi_imag = a[:, -self.N*self.N:].detach()
        phase_r = torch.sum(torch.abs(Phi_real), dim=1).reshape(-1, 1) * np.sqrt(2)
        phase_i = torch.sum(torch.abs(Phi_imag), dim=1).reshape(-1, 1) * np.sqrt(2)
        
        return phase_r, phase_i
    
    def compute_amplification(self, a):

        Amplification = a[:, 2*self.M*self.K: 2*self.M*self.K + self.N*self.N].detach()

        Amplification = torch.abs(Amplification)

        norm = torch.norm(Amplification, p='fro', dim=(1), keepdim=True)
        scaling_factor = (torch.max(norm, torch.tensor(15.0)) - 10)/(norm - 1) 


        Amplification = 1 + (Amplification * scaling_factor)

        Amplification = torch.clamp(Amplification, min=1, max=15)
    ...
    def forward(self, state):
        ...
        real_normal = real_normal.expand(-1,self.N*self.N)
        imag_normal = imag_normal.expand(-1, self.N*self.N)

        # Extract amplification matrix (real only)
        amplification = self.compute_amplification(a.detach())

        # Concatenate terms and return the normalized action
        division_term = torch.cat([current_power_t, real_normal, imag_normal, amplification], dim=1)
...
class SD3(object):
    def __init__(self, state_dim, action_dim, M, N, K, power_t, group, max_action, actor_lr, critic_lr, actor_decay, critic_decay, device, discount=0.99, tau=0.001):
        self.device = device
        powert_t_W = power_t 
        # powert_t_W = 10 ** (power_t / 10)

        self.actor = Actor(state_dim, action_dim, M, N, K, powert_t_W, group, max_action=max_action, device=device).to(self.device)
    ...
    def load(self, file_name):
        ...
        self.actor_target = copy.deepcopy(self.actor)
        print(f"Model Loaded sucessfully from {file_name}")