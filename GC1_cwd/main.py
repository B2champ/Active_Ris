...
if __name__ == "__main__":
    ...
    parser.add_argument("--num_eps", default=100, type=int, metavar='N', help='Maximum number of episodes (default: 1000)')
    ...
    # print("state_dim_____0", state_dim)
    ...
    writer = SummaryWriter(log_dir=f"./Tensorboard/{args.experiment_type}/tensorboard_logs")

    max_reward = 0

    for eps in range(int(args.num_eps)):
        state, done = env.reset(), False
        # print('in main state dim')
        ...
        for t in range(int(args.num_time_steps_per_eps)):
            ...
            next_state, reward, done, sum_rate, probing = env.step(action)
            ...
            writer.add_scalar('Reward', reward, eps * args.num_time_steps_per_eps + t)
...
#     parser.add_argument("--env", default="RIS_MISO")
#     parser.add_argument("--seed", default=0, type=int)
#     parser.add_argument("--gpu", default="0", type=int)
#     parser.add_argument("--start_time_steps", default=0, type=int)
#     parser.add_argument("--buffer_size", default=100000, type=int)
#     parser.add_argument("--batch_size", default=16, type=int)
#     parser.add_argument("--save_model", action="store_true")
#     parser.add_argument("--load_model", default="")
#     parser.add_argument("--num_antennas", default=8, type=int)
#     parser.add_argument("--num_RIS_elements", default=16, type=int)
#     parser.add_argument("--num_groups", default=4, type=int)
#     parser.add_argument("--num_users", default=5, type=int)
#     parser.add_argument("--power_t", default=mt.pow(10, (40-30)/10), type=float)
#     parser.add_argument("--num_time_steps_per_eps", default=1000, type=int)
#     parser.add_argument("--num_eps", default=100, type=int)
#     parser.add_argument("--awgn_var", default=mt.pow(10, -((170-30)/10)), type=float)
#     parser.add_argument("--channel_est_error", default=False, type=bool)
#     parser.add_argument("--exploration_noise", default=0.0)
...
#     file_name = f"{args.num_antennas}_{args.num_RIS_elements}_{args.num_users}_{args.power_t}_{args.lr}_{args.decay}"
#     if not os.path.exists(f"./Learning Curves1/{args.experiment_type}"):
#         os.makedirs(f"./Learning Curves1/{args.experiment_type}")
#     if args.save_model and not os.path.exists("./Models"):
#         os.makedirs("./Models")

#     csv_path = f"./Learning Curves1/{args.experiment_type}/{file_name}_metrics.csv"
#     with open(csv_path, mode='w', newline='') as csv_file:
#         writer = csv.writer(csv_file)
#         writer.writerow(["Episode", "Step", "Instantaneous Reward", "Avg Reward per Episode", "Sum Rate", "Avg Sum Rate per Episode", "Probing Power"])

#     env = env_GC.RIS_MISO(args.num_antennas, args.num_RIS_elements, args.num_groups, args.num_time_steps_per_eps,
#                           args.num_users, AWGN_var=args.awgn_var, carrfreq=10**9, alpha_t=2.2, alpha_r=2.2,
#                           Rican_BR=10, Rican_RU=10, Rican_BU=10, h_ris=20, h_base=10, x_bs=0, y_bs=0,
#                           x_ris=0, y_ris=150)
#     torch.manual_seed(args.seed)
#     np.random.seed(args.seed)
...
#     writer = SummaryWriter(log_dir=f"./Tensorboard1/{args.experiment_type}/tensorboard_logs")

#     for eps in range(int(args.num_eps)):
#         state, done = env.reset(), False
#         state = whiten(state)
#         episode_reward, episode_sum_rate, episode_probing_power = 0, 0, 0

#         for t in range(int(args.num_time_steps_per_eps)):
#             action = agent.select_action(np.array(state))
#             next_state, reward, done, sum_rate, prob_power = env.step(action)
#             replay_buffer.add(state, action, next_state, reward, done)

#             state = whiten(next_state)
#             episode_reward += reward
#             episode_sum_rate += sum_rate
#             episode_probing_power += prob_power

#             writer.add_scalar("Reward/Instantaneous", reward, eps * args.num_time_steps_per_eps + t)
#             writer.add_scalar("Sum Rate", sum_rate, eps * args.num_time_steps_per_eps + t)
#             writer.add_scalar("Probing Power", prob_power, eps * args.num_time_steps_per_eps + t)

#             with open(csv_path, mode='a', newline='') as csv_file:
#                 csv_writer = csv.writer(csv_file)
#                 csv_writer.writerow([eps + 1, t + 1, reward, episode_reward / (t + 1), sum_rate, episode_sum_rate / (t + 1), prob_power])

#             if done:
#                 break

#         writer.add_scalar("Reward/Average per Episode", episode_reward / (t + 1), eps + 1)
#         writer.add_scalar("Sum Rate/Average per Episode", episode_sum_rate / (t + 1), eps + 1)
#         writer.add_scalar("Probing Power/Average per Episode", episode_probing_power / (t + 1), eps + 1)

#         print(f"Episode {eps + 1} | Reward: {episode_reward:.2f}, Avg Reward: {episode_reward / (t + 1):.2f}, Sum Rate: {episode_sum_rate:.2f}")
